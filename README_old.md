...
| B2 | `code/b2_run_skill.py` | 独立运行基础 Skill。 |
...
联网搜索演示依赖 `ddgs>=9.14,<10`。当前 `web_search` 直接调用 DDGS/DuckDuckGo，不需要额外桥接服务。
...
| 参数 | 说明 |
|---|---|
| `--skill` | Skill 名称来自 `configs/tools.yaml`，例如 `calculator`、`current_time`、`file_reader`、`web_search`、`local_file_search`、`table_analyzer` 等。 |
...
| web_search | `data/tool_inputs/tool_input_web_search.json` | `query`、`top_k`、`search_type`。 | 网络不可用或无结果时返回 error SkillResult |
...
python b2_run_skill.py --skill current_time --input ../data/tool_inputs/tool_input_current_time.json --outdir ../outputs/B2_skills
...
python b2_run_skill.py --skill web_search --input ../data/tool_inputs/tool_input_web_search.json --outdir ../outputs/B2_skills
...
`web_search` 直接调用 DDGS/DuckDuckGo，返回规范化后的标题、链接、来源、发布时间和摘要。搜索结果只用于发现外部信息来源，不负责打开网页正文。

前端上传的文件会先保存到 `data/uploads/<conversation_id>/`，再以 `uploads/...` 路径传给 Agent；用户说“这份文档”时，后端会把本次上传路径作为上下文提供给 `file_reader`。
...
| `outputs/B2_skills/web_search_result.json` | JSON 对象（SkillResult） | 网页搜索结果；网络不可用或无结果时为 error。 |
...
| `data/messages/b3_tool_call_web_search.json` | 网页搜索调用样例；当前使用 DDGS/DuckDuckGo。 |
...
python b3_tool_layer.py --tools_config ../configs/tools.yaml --toolset basic_tools --tool_calls ../data/messages/b3_tool_call_web_search.json --execute --outdir ../outputs/B3_tools/web_search
...
结构化判断；失败时只写入中性占位标签和 source 引用，不用人工关键词、停用词或正则去推断语义。
B5 会读取 B1 workspace trace 中的 planning、tool observation、known facts、missing info 和 final 状态，
用于每轮结束后的模型反思。`prompt_json` 模式下，B1 在 workspace 初始化前调用
`prepare_workspace_memory_context`，B5 返回独立的 `recent_history_messages` 和 `workspace_memory`：
原始 runtime `history_messages` 保留不改，workspace 只使用最近原文；更早历史经 B5 召回后以
任务记忆、块/轮摘要、source message/tool step 片段进入 `workspace.memory.layered`。
B1 只消费该结构，不在 B1 内实现召回、压缩或语义分类。
...
| `outputs/B1_runtime/workspace_memory_context.json` | JSON 对象 | B5 为 B1 workspace 准备的显式记忆包，包含近期原文和结构化 `workspace_memory`。 |
| `outputs/B1_runtime/layered_memory_context.json` | JSON 对象 | B5 分层召回的原始结果，包含任务、块/轮摘要、source 引用和召回日志。 |
...
| `web_search_result.json` | `b2_run_skill.py` | web_search 最近一次 SkillResult。 | JSON 对象 |
...
| `skill_run_log.jsonl` | `b2_run_skill.py` | Skill 的累计运行日志。 | JSONL |
