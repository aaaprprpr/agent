...
- 新增 `data/acceptance/acceptance_demo_script.json`，设计同一新会话中的 20 个核心用户轮次和 10 个扩展轮次，完整执行预计形成至少 60 条可见 Human/AI 消息；覆盖约定建立、文档核验、计算与表格、Python 沙箱、Markdown/CSV/Word 文件产物、工具错误收束、Office 文档、实时与多工具调用、记忆更新和长期记忆召回。
- 脚本将第 1-14 轮标为验收前准备、第 15-20 轮标为现场展示；每轮记录预期工具、关键事实和 B1-B5 页面证据，并提供 B2/B3 独立演示顺序、B4 协议验收顺序和 B5 召回查询。
- 新增 `data/acceptance/acceptance_questions.txt`，只保留 30 条编号问题，供用户在同一新会话中逐条手动发送；不新增自动对话入口或执行逻辑。
- 保留 B2/B3 当前“固定预设 + 可编辑 JSON”的独立演示方式，不把它们强行依赖主对话；主对话与模块演示复用相同文档、表达式和产物主题，便于验收讲解。
- 仅更新当前 `README.md` 与 `lisn.md`。旧 README、历史验收辅助文档和讲解说明不再维护，也不作为当前实现依据。
- 本次未启动项目、未运行测试、未执行工具、未调用模型；样例中的模型措辞、工具选择、B5 后台反思和 memory block 形成情况均需按用户实际运行结果确认。

# -------------------------7.14 项目文档收口------------------------------

- 按 `B方向/TEAM_README_TEMPLATE.md` 复核根目录 `README.md`，确认保留模板要求的项目概述、模块结构、模型与数据、环境、输入配置、Demo、输出、协作和已知问题九部分。
- 删除项目侧历史 `README_old.md`、`README_712.md` 和 Vite 默认 `frontend/README.md`，根目录 `README.md` 作为唯一维护的项目 README；`B方向/` 内校方模板和教程保持不变。
- 根据用户整理后的实际文件位置，将统一验收入口改为根目录 `acceptance_questions.txt`，移除已不存在的 `data/acceptance/acceptance_demo_script.json` 等引用。
- README 补充 Qwen3.5-4B 官方 ModelScope 链接、根目录 `.env` 位置、手动启动模型代理/后端/前端的命令，以及 B2-B5 的建议验收顺序。
- 根据当前 `requirements.txt` 已移除固定 torch 版本的实际状态，README 明确 local/transformers 模式需另行安装与机器 CUDA/CPU 兼容的 PyTorch，默认 qwen_api 模式不受影响。
- 保留 `当前开发方向.md`、成员日志、`lisn.md`、样例文档和运行所需文本，它们不是重复 README；本次未启动项目、未运行测试或模型，只做文档和路径静态检查。

# -------------------------7.14 README 语气复核与展示对话自然化------------------------------

- 复核唯一 `README.md` 的人称和文档语气；正文采用“本项目/系统/模块”等客观表述，没有助手身份、自我评价或面向用户的对话式承诺。
- 移除 README 中“当前工作区位于 Windows PowerShell”的临时环境痕迹，将 `.env` 示例中的第二人称占位改为中性占位，并将验收提问部分的名称调整为更准确的“统一验收提问”。
- 重写根目录 `acceptance_questions.txt` 的 30 轮问题：问题正文不再出现“验收”或 B1-B5 内部模块术语，改为围绕“星桥”项目的日常资料整理、计算、表格、文件生成、异常处理、Office 文档、时间查询和长期协作回顾。
- 保留原有能力覆盖和轮次关系：早期信息记忆、无工具回答、文件查找/读取、计算、表格、Python、三类文件产物、失败收束、偏好更新、多工具任务和跨 20 轮以上的历史回顾。
- 本次未启动项目、未运行测试、未执行工具或调用模型，仅完成文本与格式静态检查。

# -------------------------7.14 新成员项目内部教程------------------------------

- 新增根目录 `B_agent.txt`，作为面向新加入成员的内部项目教程，不替代对外 `README.md`。
- 教程从完整 Agent 数据流出发，说明 B1-B5 职责、消息与工具协议、目录结构、前后端承接关系、配置边界、运行产物排查方式和协作红线。
- 按 B 方向 PPT 逐项核对五个模块的基础与进阶要求，区分完整实现、部分实现、系统层等价替代和未实现内容；明确 `format_converter` 工具组替代、默认 Qwen API 与指定本地模型的区别、B1 workspace 的旧版 memory 注入差异，以及 B5 legacy/SQLite 两条路径的用途。
- 结合当前源码与已有运行产物整理分阶段 workspace、文件生成下载、B3 重试/缓存/统计、B4 协议容错和 10 项通过记录、B5 轮摘要/记忆块/任务记忆/向量召回/重排等工程优化。
- 本次未启动项目、未运行测试、未执行工具、未调用模型；仅阅读校方材料、现行代码、配置、文档和已有产物，并完成文本与格式静态检查。

# -------------------------7.14 四名成员个人 README------------------------------

- 按 `B方向/PERSONAL_README_TEMPLATE.md` 的七个一级章节分别完成郭嘉、徐赫、刘锐凌、王玺尊四份个人 README，沿用当前 `姓名PERSONAL_README_TEMPLATE.md` 文件命名。
- 郭嘉文档覆盖 B1 主要负责、B2 协作部分、与王玺尊共同承担前端，以及流式运行、停止/恢复、会话提示词、后端整理、调试和文档主力工作。
- 徐赫文档聚焦 B2 的 15 个 Skill、独立运行、`format_converter` 工具组替代、增强本地检索、Office/文件能力、Python 受限执行、错误和安全边界；明确郭嘉参与部分工具整理和系统接入。
- 刘锐凌文档聚焦 B3 的 tools schema、tool_calls 校验、B2 调用、ToolMessage、函数签名补充 schema、有限重试、运行内缓存、统计、多工具和 artifact 传递。
- 王玺尊文档分别展开 B4 模型来源、prompt_json、流式与协议容错，以及 B5 legacy/SQLite、轮摘要、记忆块、任务记忆、向量召回、LLM 重排和来源证据；前端和文档工作按与郭嘉共同负责表述。
- 四份文档均说明全员参与调试、缺陷修复、文档整理和代码优化，郭嘉、王玺尊重点推进文档与代码优化；未把跨成员协作文件或团队成果写成个人独立完成。
- 静态复核了模板章节、人员分工、PPT 基础/进阶要求、命令入口、样例路径、Markdown 代码块和占位符。未启动项目、未运行测试、未执行工具、未调用模型；个人运行截图需要各成员实际运行后补入。

# -------------------------7.14 王玺尊个人 README 定向优化------------------------------

- 仅优化 `王玺尊PERSONAL_README.md`，未改动另外三名成员文档；保持个人模板规定的 1-7 章结构、现有演示命令和模块边界。
- 增加“代表性成果与核查依据”，按 B4 基础实现、B4 验收完善、B5 主要实现、B5 进阶检索、前端与工程整理五条工作主线说明本人贡献，并用源码路径、现有运行产物和代表性 Git 提交辅助核查。
- 将 B4 首轮协议演示 9 项通过 5 项、修正后现有产物 10 项全部通过的过程写入联调记录，同时限定该结果只适用于当次 qwen_api/qwen-plus 配置，未扩展为跨模型结论。
- 补充 B5 从基础文档记忆到 SQLite 分层记忆、来源追溯和可降级检索的演进过程，明确 legacy 基础演示与系统主要实现并存，不混写两条路径。
- 增加个人实现取舍，说明 B4 不越过 B1/B3 边界、B5 先保存事实再生成定位信息、模型增强能力可降级、验收页面只展示真实后端能力，以及前端整理不改变业务接口。
- 静态检查确认 7 个二级章节、40 个成对代码围栏、无模板占位符、无尾随空格，新增引用路径均存在。本次未启动项目、未运行测试、未执行工具、未调用模型；运行结果沿用用户已生成的现有产物，截图仍需本人实际运行后补充。

# -------------------------7.14 王玺尊验收攻略------------------------------

- 新增根目录 `攻略.txt`，面向王玺尊负责的 B4、B5 以及协作承担的前端、联调、文档和代码整理工作，作为验收前准备、现场操作和答辩问答手册。
- 按“长会话准备 -> B4 真实调用观察 -> B4 十项协议演示 -> B5 分层记忆观察 -> B5 早期事实召回 -> 工程说明”的顺序给出 12-15 分钟建议路线，并提供逐页按钮、观察字段和可直接使用的通俗讲解。
- 分别整理 B4/B5 基础要求、已完成进阶、部分完成项和未完成项，明确 B4 不执行工具、Plan-and-Execute 由 B1 编排、qwen-plus 不等同于课程指定 Qwen3.5-4B、legacy 文档记忆与 SQLite 主要实现用途不同。
- 收录 B4 十项用例的验收含义、B5 轮摘要/3-8 轮记忆块/任务记忆/向量召回/LLM 重排/来源证据的说明，以及 25 个常见提问和 12 类现场异常处理。
- 核对现有 B4 产物，确认 `20260714_125152_539507` 为当前最新可见的 10/10 qwen_api/qwen-plus 结果；攻略限定其为历史实际运行证据，不代替验收当天实测。
- 明确 B5 演示页“上传会话数据”当前未接通外部 JSON 导入，真实 recall-preview 仍依赖 SQLite 中已有 conversation_id；攻略要求不演示、不误报该能力。
- 静态检查攻略共 14 部分，页面按钮、配置、命令和引用路径均与当前文件一致，无占位符、尾随空格或敏感凭据。本次未启动项目、未运行测试、未执行工具、未调用模型。
