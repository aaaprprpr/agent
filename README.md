...
MCP 联网搜索桥接依赖 `mcp>=1.27,<2` 和 `ddgs[mcp]>=9.14,<10`。共享 `configs/mcp.yaml` 已配置免费的 DDGS MCP server：`ddgs mcp`，主搜索使用 `backend=auto`，失败时按配置回退到 DDGS 的 `search_news` 和 DuckDuckGo 后端。
个人需要改 MCP server 或代理时，可复制 `configs/mcp.local.example.yaml` 为被 git 忽略的 `configs/mcp.local.yaml`。`mcp_web_search` 会优先读取本地覆盖文件，没有本地覆盖时读取共享的 `configs/mcp.yaml`。
...
`mcp_web_search` 使用共享 `configs/mcp.yaml` 中的免费 `ddgs mcp` 配置；只有需要自定义 MCP server、后端或代理时，才需要复制 `configs/mcp.local.example.yaml` 为本地覆盖文件。搜索结果为空或 MCP 工具返回 `is_error=true` 时，Skill 会按 `fallbacks` 配置继续尝试，并在结果中记录 `mcp_attempts`。
...
| `configs/mcp.yaml` | 定义 MCP 搜索桥接的默认 server、transport、工具名和参数映射；默认 disabled。 |
...
| `configs/mcp.yaml` | 由 mcp_web_search 间接读取；当前共享配置使用免费的 DDGS MCP server。 |
