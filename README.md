...
| mcp_web_search | `data/tool_inputs/tool_input_mcp_web_search.json` | `query`、`top_k`；需要先配置并启用 `configs/mcp.yaml`。 | 未配置时返回结构化 error SkillResult |
...
`mcp_web_search` 默认会因为 `configs/mcp.yaml` 未启用而生成 error SkillResult；配置 MCP server 后再作为联网搜索正式演示。
...
| `data/messages/b3_tool_call_mcp_web_search.json` | MCP 搜索调用样例；未启用 MCP server 时用于验证清晰错误。 |
...
| `configs/mcp.yaml` | 由 mcp_web_search 间接读取；只有用户任务触发 MCP 搜索工具时才需要启用。 |
