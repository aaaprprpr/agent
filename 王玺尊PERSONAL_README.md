...
个人工作不以页面数量或代码行数衡量，主要以模块是否形成完整链路、关键问题是否解决以及结果是否能够追溯为准。下表列出本人负责范围内具有代表性的成果；提交编号用于辅助核查，协作文件中的改动仍按团队协作成果理解。

| 工作阶段 | 本人重点工作 | 形成的结果 | 核查依据 |
|---|---|---|---|
| B4 基础实现 | 建立模型来源适配、tools schema 注入、原始输出保存和 AIMessage 解析 | B4 能以统一接口连接 local、fastapi、qwen_api，并向 B1 返回标准消息 | `code/b4_local_agent_llm.py`；代表性提交 `8b8b3a9` |
| B4 验收完善 | 根据实际失败产物定位 JSON 协议、参数别名和流式增量问题，补充协议用例与观察接口 | 最新现有产物记录 qwen_api/qwen-plus 下 10 项用例全部通过 | `backend/b4_demo_service.py`、`outputs/backend_runs/b4_demo/20260714_110530_516977/`；代表性提交 `ae9c36f` |
| B5 主要实现 | 将基础文档记忆扩展为 SQLite 会话事实、分层反思、任务记忆和受预算约束的上下文召回 | 多轮会话具备持久化、后台反思、来源回查和可降级召回链路 | `code/b5_memory_parts/`、`code/common/conversation_store.py`；代表性提交 `242b09d`、`00b26f1`、`2c7e263` |
| B5 进阶检索 | 实现向量召回、embedding 缓存、候选约束的 LLM 重排和召回日志 | 语义检索与字段排序结合，外部服务不可用时仍可继续回答 | `vector_retrieval.py`、`rerank.py`、`retrieval.py`；代表性提交 `b1a2221`、`0ba605f` |
| 前端与工程整理 | 参与 B2-B5 页面，重点完善 B4/B5 观察和演示；拆分大型组件并归并公共展示逻辑 | 页面展示与真实后端生命周期一致，模块文件职责更清晰 | `frontend/src/B4*`、`B5ModuleView.tsx`、`moduleViewUtils.ts`；代表性提交 `b95db9a`、`22a0cbd`、`3a83fe6` |
...
| 问题 | 处理方式 | 结果 |
|---|---|---|
| 首轮 B4 协议演示 9 项仅通过 5 项 | 逐项对照 prompt、raw output 和 AIMessage，收紧 prompt_json 协议，增加 JSON mode，并只恢复可确定的格式偏差 | 用户重新运行后，现有 `20260714_110530_516977` 产物记录 10/10 通过；该结论限于当次 qwen_api/qwen-plus 配置 |
...
| 基础文档记忆无法满足浏览器多会话和精确追溯 | 保留 legacy 路径用于基础演示，新增 SQLite 保存原始消息、工具步骤、摘要、块、任务和召回日志 | 基础要求与系统主要实现并存，二者职责不再混淆 |
...
| B5 召回结果难以判断是否可靠 | 为摘要、记忆块和任务保留来源 id，召回时加载原始消息与工具步骤并记录检索过程 | 能从摘要定位回原始事实，而不是只凭压缩文本判断 |
...
- **B4 只做协议层职责。** B4 不根据业务关键词替 B1 选择工具，也不执行 B3 工具。解析器只修复字段别名、纯文本等能够确认语义的偏差；无法判断的损坏 JSON 仍返回错误，避免悄悄改变模型意图。
- **B5 先保存事实，再生成定位信息。** 原始消息和工具步骤是事实来源，摘要、记忆块和任务记忆用于定位与筛选。需要精确事实时回查来源，避免压缩内容覆盖原始记录。
- **模型增强能力必须可降级。** 反思、embedding 和 LLM 重排可以提高质量，但不应成为主对话的单点故障。对应服务不可用时保留原始事实并回退字段排序，使回答链路仍能继续。
- **验收页面只展示真实能力。** B4 页面读取真实模型调用和协议结果；B5 页面读取当前会话记忆并执行真实召回。没有独立后端生命周期的“即时压缩”按钮被移除，防止界面演示与实现不一致。
- **优化服从模块边界。** 前端拆分和公共函数整理只处理展示层重复代码，不改变 B1-B5 接口；涉及协作模块的修正以最小改动完成，并保留 Git 和运行产物作为复核依据。
