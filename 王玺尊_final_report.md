...
| :---: | :---: | :---: | :--- | :--- |
| **组长** | **王玺尊** | **[20236481]** | **B4 Agent LLM 决策、B5 记忆文档存储与查找；前端协作** | **[当前集成源码](https://github.com/aaaprprpr/agent)**；`[如有独立个人仓库，请补充]` |
...
```mermaid
flowchart LR
    U[用户] <--> FE[React 前端<br/>王玺尊、郭嘉协作]
    FE <--> API[FastAPI 后端]
    API --> B1[B1 Agent Runtime<br/>郭嘉]

    B1 --> B3[B3 Tool Layer<br/>刘锐凌]
    B3 --> B2[B2 Skills<br/>徐赫]
    B2 --> EXT[文件 / 计算 / 表格 / 搜索 / 沙箱]
    EXT --> B2
    B2 --> B3
    B3 --> B1

    B1 <--> B4[B4 LLM 决策与协议层<br/>王玺尊]
    B4 <--> MS[Qwen API / FastAPI / 本地模型]

    B1 <--> B5[B5 分层记忆与召回<br/>王玺尊]
    B5 <--> DB[(SQLite 会话事实与记忆)]
    B5 -.反思 / Rerank.-> B4
...
仓库当前本地运行目录中保存了一次可核查的 B4 协议演示结果：

- **产物路径**：`outputs/backend_runs/b4_demo/20260714_110530_516977/b4_protocol_test_result.json`
- **模型源**：`qwen_api`
- **模型**：`qwen-plus`
- **工具绑定方式**：`prompt_json`
- **当次结果**：10 项通过，0 项失败
...
| 用例 | 类型 | 结果 | 当次耗时（ms） |
|---|---|---:|---:|
| `content_response` | 真实模型 | 通过 | 915.015 |
| `single_tool_call` | 真实模型 | 通过 | 1251.434 |
| `multiple_tool_calls` | 真实模型 | 通过 | 1662.723 |
| `multiple_tool_messages` | 真实模型 | 通过 | 1137.018 |
| `error_tool_message` | 真实模型 | 通过 | 962.687 |
| `stream_response` | 真实模型 | 通过，收到 28 个 delta | 2233.607 |
| `content_with_tool_call` | 解析器 | 通过 | 0.047 |
| `normalize_parameters_alias` | 解析器 | 通过 | 0.024 |
| `recover_trailing_markers` | 解析器 | 通过 | 0.049 |
| `reject_empty_message` | 解析器 | 通过 | 0.016 |

六个真实模型用例的单次平均耗时为 **1360.414 ms**，范围为 **915.015–2233.607 ms**；四个纯解析用例平均耗时约 **0.034 ms**。
...
- `[提交前插入：B4 10/10 协议用例汇总截图]`
- `[提交前插入：展开 multiple_tool_calls 或 stream_response 的详细证据]`
- `[提交前插入：B5 长对话召回结果，必须同时显示 source messages/tool steps]`
...
- **个人独立代码库**：`[提交前补充；若无独立仓库，需向验收教师确认团队仓库模块链接是否满足要求]`
...
1. 填写王玺尊本人学号。
2. 填写刘锐凌、徐赫学号；确认郭嘉学号 `20236529` 无误。
3. 补充本人个人独立代码库链接；如无独立仓库，向教师确认团队仓库内的 B4/B5 直接链接是否满足要求。
4. 补充其他成员个人仓库链接，或确认成员表只要求本人链接。
5. 使用最终 commit 和最终验收模型重新运行，确认是否仍使用 `qwen_api/qwen-plus`；若改为 Qwen3.5-4B，更新模型说明和 B4 结果。
6. 插入 2–3 张团队 Demo 截图及 B4/B5 个人模块结果截图。
7. 确认 `outputs/backend_runs/b4_demo/20260714_110530_516977/` 是否会随最终提交保留；若不保留，应导出截图或把结果摘要复制到正式验收材料。
8. 最终检查所有 `[提交前补充]` 和 `[提交前插入]` 标记均已处理。
